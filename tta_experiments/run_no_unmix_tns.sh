...
for dataset in ${datasets[*]}; do
      ...
      then
            default_options="TEST.BATCH_SIZE 16 TEST.DELTA_DIRICHLET 0.01 OPTIM.LR 2.5e-6"
      fi
      ...
      for setting in ${settings[*]}; do
            ...
            for method in ${methods[*]}; do
                  for seed in ${seeds[*]}; do
                        ...
                        python test_time.py --cfg $config SETTING $setting RNG_SEED $seed ${options} ${default_options} ${mix_k_options} SAVE_DIR ./output_unmixtns/${setting} UNMIXTNS.USE_BN True
                  done
            done
      done
done