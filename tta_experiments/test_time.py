...
# introduce UnMix TNS here
from unmixtns import replace_bn_layers
...
def evaluate(description):
    ...
    # replace bn layers with unmix tns layers
    if not cfg.UNMIXTNS.USE_BN:
        print("Replacing BatchNorm layers with UnMix-TNS layers")
        replace_bn_layers(base_model, cfg.UNMIXTNS.NUM_COMPONENTS, cfg.TEST.BATCH_SIZE , cfg.UNMIXTNS.BATCH_SIZE_MAX )
    else:
        print("Using original BatchNorm layers")
    ...
    with open(os.path.join(cfg.SAVE_DIR, "results.pkl"), 'wb') as f:
